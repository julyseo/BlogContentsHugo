...
travis 사이트에 로그인하여 Account Setting 의 Legacy Services Integration 에서   
컨텐츠 관리 repo 의 스위치를 on 시켜준다.

Travis CI는 repository 의 root 디렉토리에 .travis.yml 을 읽어들여 수행한다.  
travis 사이트에서 설정했다면 .travis.yml 을 작성하여 빌드/배포를 수행하도록 할 수 있다.

여기에서는 Hugo 로 빌드하고 호스팅 될 repository 에 배포하기 위해 .travis.yml 를 작성한다.  
자동화할 항목은 
1. Hugo build
2. deploy to target repo 정도 인데, 2번 과정에서 github token 이 필요한 것 같다.

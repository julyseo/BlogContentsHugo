...
위의 버전 관리 시스템의 설명으로 대략적인 방식을 알 수 있지만 굉장히 원론적이다.  
...
~~**Git이 버전 관리 시스템인건 알겠다고. 그니까 Git 이 뭐냐고?!**~~
...
Git이 뭔지는 아무래도 상관없고,  
어떤 원리로 버전 관리를 하는지, 왜 git을 쓰는지 살펴보자. (~~그럼 답이 나올거 같아~~)
...
*(혹시 스냅샷이 무슨 뜻인지 감이 안온다면 "파일들을 캡쳐해놓는다" 정도로 생각하면 된다.)*  
...
그 뒷버전 부터 파일 뒤에 붙는 숫자는 수정된 버전..  
근데 테투리에 빗금 표시된 애들은 뭘까?

버전이 몇개가 있고, 버전마다 파일이 생성되거나 변화하는데,  
당연히 변경내용이 없으면 버전을 만들 수 없다.  
...
라는 식으로 관리하는 거다.  
**빗금 테두리는 바뀐거 없는 파일**인 셈이다.

그래도 히스토리가 쌓이거나 용량이 큰 파일이 한번 들어오면  
버전업이 될 때마다 저장소의 크기가 증가한다.  
그러나 기본적으로 압축 알고리즘을 통해 저장하기 떄문에,  
...
영역들에 무언가가 존재할 것이며 명령어로 그 무언가의 위치를 바꿀 수 있는 것 같다!

그렇다면 그 무언가가 뭘까?  
일단 git 은 파일들의 스냅샷을 저장하기 때문에 당연히 파일일 것이고,  
그 중에서도 변경사항이 있는 파일이 해당할 것이다.

### 영역의 의미
이제 위의 그림에서 나온 각 영역에 대한 의미를 알아볼 것이다.

#### 로컬 영역, 그리고 세 가지 상태
지금까지의 설명은 git 을 버전 관리를 하는 도구로써 바라봤지만,  
사실 git 은 버전 관리 이전에 로컬의 파일 상태도 관리한다.  
이 상태들은 로컬의 3가지 영역과 1:1 로 매칭된다.
...
##### Staged : 준비 영역 (Staged Area)
Index 라고도 불리며, **로컬 저장소에 반영(Commit)하기 전의 준비 영역**이다.  
...
(그런데, Staged Area 를 포함하는 개념인지는 모르겠다)  
...
**로컬 영역에는 존재하는 작업 영역이 bare 저장소엔 없다.**

---
꽤 많은 내용을 다룬 것 같지만 빠진 내용이 몇개 있다.  
.git 의 구조 및 요소 설명이라던지, 브랜치, 명령어 설명 등..
...
어느정도 익숙해지고 나면 더 공부하는 것도 좋을 것 같다.